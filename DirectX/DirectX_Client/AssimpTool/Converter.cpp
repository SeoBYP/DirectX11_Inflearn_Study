...
// 주어진 파일 경로에서 에셋 파일을 읽어오는 함수
void Converter::ReadAssetFile(wstring file)
{
	wstring fileStr = _assetPath + file; // 전체 파일 경로 생성
	auto p = std::filesystem::path(fileStr);
	assert(std::filesystem::exists(p)); // 파일이 실제로 존재하는지 확인

	// Assimp를 사용해 파일을 읽어오는 과정. 여러 가공 처리를 함께 수행
	_scene = _importer->ReadFile(
		Utils::ToString(fileStr),
		aiProcess_ConvertToLeftHanded | // 좌표계 변환
		aiProcess_Triangulate | // 모든 메시를 삼각형으로 변환
		aiProcess_GenUVCoords | // UV 좌표 생성
		aiProcess_GenNormals | // 법선 생성
		aiProcess_CalcTangentSpace // 탄젠트 공간 계산
	);

	assert(_scene != nullptr); // 읽기 성공 확인
}
...
{
	wstring finalPath = _modelPath + savePath + L".clip"; // 최종 파일 경로
	assert(index < _scene->mNumAnimations); // 유효한 애니메이션 인덱스 확인
	shared_ptr<asAnimation> animation = ReadAnimationData(_scene->mAnimations[index]); // 애니메이션 데이터 읽기
	WriteAnimationData(animation, finalPath); // 애니메이션 파일 쓰기
}
	...
	for (uint32 i = 0; i < _scene->mNumMeshes; i++) {
		...
		for (uint32 b = 0; b < srcMesh->mNumBones; b++) {
			...
			for (uint32 w = 0; w < srcMeshBone->mNumWeights; w++) {
				...
				tempVertexBoneWeights[index].AddWeight(boneIndex, weight); // 가중치 정보 추가
			}
		}
		...
		for (uint32 v = 0; v < tempVertexBoneWeights.size(); v++)
		{
			...
			mesh->vertices[v].blendIndices = blendWeight.indeices; // 본 인덱스 설정
...
shared_ptr<asAnimationNode> Converter::ParseAnimationNode(shared_ptr<asAnimation> animation, aiNodeAnim* srcNode)
{
	...
	for (uint32 k = 0; k < keyCount; k++) 
	{
		...
		asKeyframeData frameData; // 키프레임 데이터 객체

		bool found = false; // 키프레임이 발견되었는지 여부
		uint32 t = node->keyframe.size(); // 현재 키프레임의 인덱스
	...
	if (node->keyframe.size() < animation->frameCount) {
		uint32 count = animation->frameCount - node->keyframe.size(); // 채워야 할 키프레임 수
...
// 주어진 본 이름에 해당하는 본의 인덱스를 반환하는 함수
uint32 Converter::GetBoneIndex(const string& name)
{
	// 모든 본을 순회하며 이름이 일치하는 본을 찾습니다.
	for (shared_ptr<asBone>& bone : _bones) {
		if (bone->name == name)
			return bone->index; // 일치하는 본의 인덱스를 반환
	}
	assert(false); // 이름이 일치하는 본을 찾지 못한 경우, 에러를 발생시킵니다.
	return 0; // 이 부분은 실행되지 않습니다.
}
