...
// 색상 데이터를 가진 사각형(쿼드)을 생성합니다.
// 이 함수는 사각형(두 개의 삼각형으로 구성된 직사각형)을 초기화하고, 각 정점에 위치와 색상 데이터를 할당합니다.
// geometry: 기하 구조체로, 정점과 색상 정보를 담습니다.
// color: 정점에 적용할 색상입니다.
void GeometryHelper::CreateQuad(shared_ptr<Geometry<VertexColorData>> geometry, Color color)
{
	vector<VertexColorData> vtx;
	vtx.resize(4); // 사각형을 구성하는 4개의 정점

	// 정점 위치 설정
	vtx[0].position = Vec3(-0.5f, -0.5f, 0.f); // 왼쪽 아래
	vtx[1].position = Vec3(-0.5f, 0.5f, 0.f);  // 왼쪽 위
	vtx[2].position = Vec3(0.5f, -0.5f, 0.f);  // 오른쪽 아래
	vtx[3].position = Vec3(0.5f, 0.5f, 0.f);   // 오른쪽 위

	// 색상 할당
	for (int i = 0; i < 4; ++i) {
		vtx[i].color = color; // 모든 정점에 동일한 색상 적용
	}

	// 기하 구조체에 정점 데이터 설정
	geometry->SetVertices(vtx);

	// 인덱스 버퍼 초기화(두 개의 삼각형으로 사각형 구성)
	vector<uint32> idx = { 0, 1, 2, 2, 1, 3 };
	geometry->SetIndices(idx);
}

// 텍스처 데이터를 가진 사각형을 생성합니다.
// 이 함수는 사각형을 초기화하고, 각 정점에 위치와 UV 텍스처 좌표를 할당합니다.
// geometry: 기하 구조체로, 정점 위치와 UV 좌표 정보를 담습니다.
void GeometryHelper::CreateQuad(shared_ptr<Geometry<VertexTextureData>> geometry)
{
	...
	// 정점 위치와 UV 좌표 설정
	vtx[0].position = Vec3(-0.5f, -0.5f, 0.f); vtx[0].uv = Vec2(0.f, 1.f); // 왼쪽 아래
	vtx[1].position = Vec3(-0.5f, 0.5f, 0.f);  vtx[1].uv = Vec2(0.f, 0.f); // 왼쪽 위
	vtx[2].position = Vec3(0.5f, -0.5f, 0.f);  vtx[2].uv = Vec2(1.f, 1.f); // 오른쪽 아래
	vtx[3].position = Vec3(0.5f, 0.5f, 0.f);   vtx[3].uv = Vec2(1.f, 0.f); // 오른쪽 위
...
// 텍스처 데이터를 가진 큐브를 생성합니다.
// 이 함수는 큐브를 초기화하고, 각 면의 정점에 위치와 UV 텍스처 좌표를 할당합니다.
// geometry: 기하 구조체로, 정점 위치와 UV 좌표 정보를 담습니다.
void GeometryHelper::CreateCube(shared_ptr<Geometry<VertexTextureData>> geometry)
{
	...
	vector<VertexTextureData> vtx(24); // 큐브를 구성하는 24개의 정점 (각 면마다 4개의 정점)

	// 각 면(앞, 뒤, 위, 아래, 왼쪽, 오른쪽)의 정점 위치와 UV 좌표 설정
	// 앞면
	...
	// 인덱스 버퍼 초기화(각 면마다 두 개의 삼각형으로 구성)
...
// 텍스처 데이터를 가진 구를 생성합니다.
// 이 함수는 구를 초기화하고, 각 정점에 위치와 UV 텍스처 좌표를 할당합니다.
// geometry: 기하 구조체로, 정점 위치와 UV 좌표 정보를 담습니다.
void GeometryHelper::CreateSphere(shared_ptr<Geometry<VertexTextureData>> geometry)
{
	...
	vector<VertexTextureData> vtx; // 정점 데이터 컨테이너

	VertexTextureData v;

	// 북극과 남극을 포함한 모든 정점의 위치와 UV 좌표를 계산하고 설정
	// 북극
...
// 텍스처 데이터를 가진 그리드를 생성합니다.
// 이 함수는 그리드를 초기화하고, 각 정점에 위치와 UV 텍스처 좌표를 할당합니다.
// geometry: 기하 구조체로, 정점 위치와 UV 좌표 정보를 담습니다.
// sizeX, sizeZ: 그리드의 가로 및 세로 크기입니다.
void GeometryHelper::CreateGrid(shared_ptr<Geometry<VertexTextureData>> geometry, int32 sizeX, int32 sizeZ)
{
	vector<VertexTextureData> vtx; // 정점 데이터 컨테이너
	// 그리드를 구성하는 각 정점의 위치와 UV 좌표를 계산하고 설정
	...
	geometry->SetVertices(vtx);
	// 인덱스 버퍼 초기화(그리드를 구성하는 삼각형 메쉬 설정)
	vector<uint32> idx;
