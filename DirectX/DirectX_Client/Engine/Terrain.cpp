...
bool Terrain::Pick(int32 screenX, int32 scrennY, Vec3& pickPos, float& distance)
{
	...
	Vec3 n = vp.Unproject(Vec3(screenX, scrennY, 0), W, V, P);
	Vec3 f = vp.Unproject(Vec3(screenX, scrennY, 1), W, V, P);
	...
	const auto& vertieces = _mesh->GetGeometry()->GetVertices();
	...
	for (int32 z = 0; z < _sizeZ; z++) {
		for (int32 x = 0; x < _sizeX; x++) {
			...
			Vec3 p[4];
			for (int32 i = 0; i < 4; i++)
				p[i] = vertieces[index[i]].position;
			//  [2]
			//   |	\
			//  [0] - [1]
